...
class TestInitialization:
    ...
    def test_custom_params(self):
        ...
        assert helper.global_str_square_file_store_url_base == "https://192.168.1.100:8080"


class TestUploadFileUsingFilePath:
    """Tests for upload_file_using_file_path_v0 method"""

    @patch('square_file_store_helper.main.make_request_json_output')
    @patch('builtins.open', new_callable=mock_open, read_data=b'file content')
    @patch('os.path.basename')
    @patch('mimetypes.guess_type')
    def test_success(self, mock_guess_type, mock_basename, mock_file, mock_request, helper):
        ...
        result = helper.upload_file_using_file_path_v0(
            ...
            app_id=1,
            system_relative_path="uploads/docs"
        )
    ...
    @patch('os.path.basename')
    @patch('mimetypes.guess_type')
    def test_unknown_mimetype(self, mock_guess_type, mock_basename, mock_file, mock_request, helper):
        ...
        result = helper.upload_file_using_file_path_v0(file_path="/path/to/test.unknown")
    ...
    @patch('os.path.basename')
    @patch('mimetypes.guess_type')
    def test_default_system_path(self, mock_guess_type, mock_basename, mock_file, mock_request, helper):
...
class TestUploadFileUsingTuple:
    ...
    def test_success(self, mock_request, helper):
        ...
        result = helper.upload_file_using_tuple_v0(
            file=file_tuple,
            app_id=2,
            system_relative_path="uploads/pdfs"
        )
...
class TestDownloadFile:
    ...
    def test_success_with_filename(self, mock_file, mock_mkdir, mock_exists, mock_get, mock_parse, helper):
    ...
    @patch('builtins.open', new_callable=mock_open)
    def test_success_with_filename_star(self, mock_file, mock_exists, mock_get, mock_parse, helper):
        ...
        mock_content_disposition = Mock()
        mock_content_disposition.has.side_effect = lambda x: x == "filename*"
        mock_content_disposition.__getitem__ = Mock(return_value="UTF-8''test%20file.pdf")
        mock_headers.content_disposition = mock_content_disposition
        mock_parse.return_value = mock_headers

        result = helper.download_file_v0(
            file_storage_token="token456",
            output_folder_path="/tmp/downloads"
        )
    ...
    @patch('os.mkdir')
    @patch('builtins.open', new_callable=mock_open)
    def test_folder_already_exists(self, mock_file, mock_mkdir, mock_exists, mock_get, mock_parse, helper):
    ...
    @patch('os.path.exists', return_value=False)
    @patch('os.mkdir')
    def test_missing_filename(self, mock_mkdir, mock_exists, mock_get, mock_parse, helper):
    ...
    def test_http_404_error(self, mock_get, helper):
        ...
        with pytest.raises(requests.HTTPError):
            helper.download_file_v0(
                file_storage_token="invalid_token",
                output_folder_path="/tmp/downloads"
            )
    ...
    def test_various_http_errors(self, mock_get, status_code, error, helper):
        """Test file download with various HTTP error codes"""
        mock_response = Mock()
